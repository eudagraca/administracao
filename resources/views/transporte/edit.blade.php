...
@endsection
@section('links')
    <a href="{{ route('transportes.index') }}" class="uk-button uk-button-text" >Transportes</a> / Actualização
@endsection
@section('link')
    Editar dados do transporte
@endsection
    ...
    <div class="conntainer">
        ...
        <div class="uk-section uk-padding uk-section-small uk-flex uk-flex-center uk-width-1-1@s">
            <div class="uk-card uk-card-default uk-width-1-1@s">
                <form method="POST" action="{{ route('transportes.update', $transporte->id) }}" class="uk-form-stacked">
                    {{ csrf_field() }}
                    @method('PUT')
                    <div class="uk-card-body uk-margin  uk-grid">
                        <div class="uk-width-1-2@s uk-margin">
                            ...
                            <select name="veiculo" class="uk-select @error('veiculo') uk-form-danger @enderror">
                                ...
                                @foreach ($viaturas as $viatura)
                                    <option
                                        value="{{ $viatura->name }}" {{ $transporte->veiculo ==$viatura->name ? 'selected':'' }}>
                                        {{$viatura->name}}</option>
                                @endforeach
                            </select>
                            @error('veiculo')
                        ...
                        <div class="uk-width-1-2@s">
                            ...
                            <div class="uk-form-control">
                                <input class="uk-input @error('marca') uk-form-danger @enderror" id="marca" name="marca"
                                       type="text" value="{{ $transporte->marca }}" required autocomplete="marca"
                                       autofocus>
                                @error('marca')
                                <span class="uk-text-danger uk-text-small">{{ $message }}</span>
                                @enderror
                        ...
                        <div class="uk-width-1-2@s">
                            ...
                            <div class="uk-form-control">
                                <input class="uk-input @error('modelo') uk-form-danger @enderror" id="modelo"
                                       name="modelo"
                                       type="text" value="{{ $transporte->modelo }}" required autocomplete="modelo"
                                       autofocus>
                                @error('modelo')
                                <span class="uk-text-danger uk-text-small">{{ $message }}</span>
                                @enderror
                        ...
                        <div class="uk-width-1-2@s">
                            ...
                            <div class="uk-form-control">
                                <input id="matricula" type="text"
                                       class="uk-input @error('matricula') uk-form-danger @enderror" name="matricula"
                                       required value="{{ $transporte->matricula }}"
                    ...
                    <div class="uk-form-control uk-card-footer">
                        <button type="submit" class="uk-button uk-align-right uk-border-rounded uk-button-primary">
                            {{ __('Actualizar dados do transporte') }}
                        </button>
                    </div>
                </form>
            </div>
